...
% 设 $\operatorname{FT}(r_1)=u$, 构造以 $u$ 为首项的对称多项式 $g_u$(书上 P192 式(6)), 容易验证 $g_u$ 是齐次的且 $\deg g_u=\deg r_1,\therefore r_2=r_1-g_u$ 是对称, 齐次的, 且 $\deg r_2=\deg r=\deg(S(v)-g_v)$.
...
% \end{equation}
% 满足
% \[r_n=r_{n-1}-g_{\operatorname{FT}(r_{n-1})},\quad\operatorname{FT}(r_{n-1})\succ\operatorname{FT}(r_n).\]
...
\end{proof}
\begin{exercise}[4.3]\label{ex4.3}
